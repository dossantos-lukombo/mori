export default {
  ...
  getters: {
    getMessages:
      ({ newChatMessages, newGroupChatMessages }, getters, { id }) =>
      (receiverId, type) => {
        let messages = [];
    ...
    getUnreadMsgsCountFromDB: (state) => (userId) => {
      ...
      const userMsgObj = state.unreadMsgsStatsFromDB.find(
        (msg) => msg.id === userId
      );
  ...
  mutations: {
    ...
    updateWebSocketConn(state, socket) {
      state.socket = socket; // Add WebSocket connection to the state
    },
  },
  ...
  actions: {
    ...
    addNewChatMessage({ commit, state }, payload) {
      const newMessages =
        payload.type === "PERSON"
          ...
          : [...state.newGroupChatMessages, payload];
    ...
    async fetchChatUserList({ rootState, commit, dispatch }) {
      ...
      const response = await fetch(
        "http://localhost:8081/chatList?userId=" + rootState.id,
        {
          credentials: "include",
        }
      );
