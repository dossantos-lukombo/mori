...
func LoginHandler(db *sql.DB) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        if r.Method == http.MethodPost {
            // Le middleware CSRF de gorilla vérifiera automatiquement le token
            ...
            userID := user.ID
            _, err = db.Exec(`UPDATE users SET session = $1 WHERE id = $2;`, sessionToken, userID)
            if err != nil {
            ...
            _, err = db.Exec(`UPDATE conversations SET conversation_uuid = $1 WHERE user_id = $2;`, chatSessionToken, userID)
            if err != nil {
